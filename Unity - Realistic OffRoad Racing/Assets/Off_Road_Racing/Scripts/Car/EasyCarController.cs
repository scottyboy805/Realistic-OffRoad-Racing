...
namespace ALIyerEdon
{
	...
	public class EasyCarController : MonoBehaviour
	{
		...
		private void Awake()
        {
			...
			GameObject.Find("Global Volume").GetComponent<Volume>().sharedProfile.
						TryGet<UnityEngine.Rendering.HighDefinition.MotionBlur>(out motionBlur);
		}
        void Start()
		{
		...
		public void Toggle_FrontLights(bool state)
		{
			for (int a = 0; a < frontLights.Length; a++)
			{
				if (state)
				{
					frontLights[a].intensity = 2000000f;
		...
		void BrakeLights(float value)
		{
			...
			{
				brakeLights[a].intensity = value * 100000f;
		...
		void ReverseLights(float value)
		{
			...
			{
				reverseLights[a].intensity = value * 100000f;
